...
from sqlalchemy import ColumnExpressionArgument, Sequence, select, Select
...
def build_query_options[T: DeclarativeBase](
        ...
        max_depth: int) -> list[ExecutableOption]:
...
class CrudRepository[T: DeclarativeBase](BaseCrudRepository[T, int]):
    ...
    def __init__(self, session: AsyncSession, model_cls: type[T]) -> None:
        super().__init__(session=session, model_cls=model_cls) 

    async def get_by_id(self,
                        id: int, 
                        id_field: str = "id",
                        options: Iterable[ExecutableOption] | None = None) -> T | None:
        ...
        if not id_column:
            raise RuntimeError(f"id_column {id_column=} does not exist on Entity {db_class.__name__}")
    ...
    async def save(self, entity: T) -> T:
        ...
        instance = await self.session.merge(entity)
        return instance
    ...
    async def get_many(self, 
                       where_clause: Iterable[ColumnExpressionArgument] | None = None,
    ...
    async def get_by_id_with_options(self, 
                                     id: int,
                                     response_model: type[BaseModel],
                                     exclude_fields: list[str], 
                                     max_depth: int,
        ...
        result = await self.get_by_id(id=id, id_field=id_field, options=options)

        return result
    
    async def get_multi_with_options(self, 
                                     response_model: type[BaseModel],
                                     exclude_fields: list[str], 
                                     max_depth: int,
                                     where_clause: Iterable[ColumnExpressionArgument] | None = None,
